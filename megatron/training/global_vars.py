...
def _set_wandb_writer(args):
    ...
    if getattr(args, 'wandb_project', '') and args.rank == (args.world_size - 1):
        ...
        # Resume or fork wandb run based on checkpoint state.
        ...
        if os.path.isfile(wandb_id_file):
            ...
            saved_run_id = lines[0]
            wandb_kwargs['id'] = saved_run_id
            wandb_kwargs['resume'] = 'allow'
        if args.wandb_entity:
            wandb_kwargs['entity'] = args.wandb_entity
        os.makedirs(wandb_kwargs['dir'], exist_ok=True)
        wandb.init(**wandb_kwargs)
        # Save run ID and initial step (step updated during training).
