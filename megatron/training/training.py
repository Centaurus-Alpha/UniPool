...
from megatron.core.transformer.moe.moe_utils import (
    track_moe_metrics,
    track_depth_router_weight_metrics,
    track_integrated_mode_metrics,
)
...
def training_log(
    ...
    num_zeros_in_grad,
    max_attention_logit,
    moe_relu_sparsity, moe_relu_l1_reg_coeff
):
    ...
    if args.num_experts is not None:
        ...
        # Track MoD (Mixture of Depths) router auxiliary loss
        if getattr(args, 'staged_mode_enabled', False) and getattr(args, 'mod_aux_loss_coeff', 0):
            track_names.append("mod_router_aux_loss")
        # Track shared_virtual pool metrics
        if getattr(args, 'moe_expert_pool_mode', None) == 'shared_virtual':
            track_names.append("shared_virtual_pool/physical_load_cv")
    ...
    # Track depth router weight metrics for Staged MoDE (every 500 iterations)
    if getattr(args, 'staged_mode_enabled', False) and iteration % 500 == 0:
        track_depth_router_weight_metrics(
            iteration=iteration,
            writer=writer,
            wandb_writer=wandb_writer,
    ...
    # Track Integrated MoDE null expert metrics (every 500 iterations)
    if getattr(args, 'integrated_mode_enabled', False) and iteration % 500 == 0:
        track_integrated_mode_metrics(
            iteration=iteration,
            writer=writer,
            wandb_writer=wandb_writer,
    ...
    # Update wandb run ID file with current iteration for resume/fork logic.
    if wandb_writer and args.save:
        wandb_id_file = os.path.join(args.save, 'wandb_run_id.txt')
        with open(wandb_id_file, 'w') as f:
            f.write(f"{wandb_writer.run.id}\n{iteration}")
...
def train(
    ...
    while iteration < args.train_iters:
        ...
        if config.moe_relu_routing:
            ...
            # Count actual number of MoE layers (not dense layers) for correct averaging
            num_moe_layers = config.num_layers
            if hasattr(config, 'moe_layer_freq') and isinstance(config.moe_layer_freq, int) and config.moe_layer_freq > 1:
                num_moe_layers = config.num_layers // config.moe_layer_freq
            config.moe_relu_sparsity /= num_moe_layers * num_microbatches
            ...
            # average across tensor parallel and expert parallel ranks to ensure consistency
            torch.distributed.all_reduce(config.moe_relu_sparsity, group=mpu.get_tensor_model_parallel_group(), op=torch.distributed.ReduceOp.SUM)
            config.moe_relu_sparsity /= mpu.get_tensor_model_parallel_world_size()
            if mpu.get_expert_model_parallel_world_size() > 1:
                torch.distributed.all_reduce(config.moe_relu_sparsity, group=mpu.get_expert_model_parallel_group(), op=torch.distributed.ReduceOp.SUM)
                config.moe_relu_sparsity /= mpu.get_expert_model_parallel_world_size()
            # zeroth-order update to the coefficient based on the average sparsity
            # Use staged_mode_moe_router_topk if available (for Staged ReMoDE)
            effective_topk = config.moe_router_topk
            if getattr(config, 'staged_mode_enabled', False) and getattr(config, 'staged_mode_moe_router_topk', None) is not None:
                effective_topk = config.staged_mode_moe_router_topk
            target_sparsity = 1 - effective_topk / config.num_moe_experts
            if config.moe_relu_sparsity < target_sparsity:
                config.moe_relu_l1_reg_coeff *= config.moe_relu_l1_reg_coeff_multiplier
            else:
                config.moe_relu_l1_reg_coeff /= config.moe_relu_l1_reg_coeff_multiplier
            # Broadcast moe_relu_l1_reg_coeff from rank 0 to ensure all ranks have the same value
            # This prevents floating-point accumulation errors from causing divergence
            torch.distributed.broadcast(config.moe_relu_l1_reg_coeff, src=0)
...
def evaluate(
    ...
    args = get_args()
    timers = get_timers()
    # `args.iteration` is only refreshed at checkpoint load, not during the
    # training loop, so callers in train() pass the real step explicitly.
    ...
    # Shared-pool MoE routing diagnostics: attach hooks (idempotent) and
    # start accumulating tokens_per_expert on the validation set. Fail-safe;
    # never affects eval loss. See megatron/core/transformer/moe/routing_diagnostics.py.
    for model_module in model:
        _routing_diag.register(model_module)
    _routing_diag.start_recording()
    ...
    with torch.no_grad():
        # Local eval microbatch counter; named distinctly so it does not shadow
        # the function-parameter `iteration` (the real training step) which
        # finalize_and_log() consumes below.
        eval_step = 0
        if verbose:
            print_rank_0(f'Evaluating on {eval_iters * eval_batch_size} samples')
        while eval_step < eval_iters:
            eval_step += 1
            if verbose:
                print_rank_0(f'Evaluating iter {eval_step}/{eval_iters}')
