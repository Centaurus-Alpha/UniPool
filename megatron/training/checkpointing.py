...
def check_checkpoint_args(checkpoint_args):
    ...
    # Validate expert pool configuration if MoE is used
    if getattr(args, 'num_experts', None) is not None:
        _compare('moe_expert_pool_mode', default='shared')
        _compare('moe_expert_pool_size', default=1)
        _compare('moe_norm_routing', default=False)
        if args.moe_norm_routing or getattr(checkpoint_args, 'moe_norm_routing', False):
            _compare('moe_norm_routing_init_method', default='monte_carlo')
            _compare('moe_norm_routing_eps', default=1e-8)
...
def load_args_from_checkpoint(
    ...
    # Expert pool configuration (CRITICAL for hyper pool mode)
    _set_arg('moe_expert_pool_mode', force=True)
    _set_arg('moe_expert_pool_size', force=True)

    # Routing configuration (CRITICAL - must match training)
    _set_arg('moe_norm_routing', force=True)
    _set_arg('moe_norm_routing_init_method', force=True)
    _set_arg('moe_norm_routing_eps', force=True)

    # Additional MoE routing types
    _set_arg('moe_layer_pool_size', force=True)
