...
class GPTModel(LanguageModule):
    ...
    def __init__(
        ...
        # Special handling for ReMoE (ReLU routing)
        # NOTE: Must be done BEFORE decoder construction so that ReLURouter can access moe_relu_l1_reg_coeff
        if self.config.moe_relu_routing:
            moe_relu_l1_reg_coeff = torch.tensor(
                self.config.moe_relu_l1_reg_coeff_init,
                dtype=torch.float32,
            ...
            moe_relu_sparsity = torch.zeros(
                1, dtype=torch.float32, device=torch.cuda.current_device(), requires_grad=False
            )
