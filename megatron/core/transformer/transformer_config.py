...
class TransformerConfig(ModelParallelConfig):
    ...
    moe_expert_pool_size: int = 1
    """Number of adjacent MoE layers that share an expert pool. Default 1 (no pooling).
    When > 1, every pool_size adjacent MoE layers share experts. The sharing mode is
    controlled by moe_expert_pool_mode."""

    moe_expert_pool_mode: str = 'shared'
    """Mode for expert pool sharing across adjacent MoE layers. Options:
    - 'shared': All layers in a pool share one set of (pool_size * num_experts) experts.
      num_experts is automatically multiplied by pool_size internally. Parameters are shared.
    - 'shared_no_tie': Same as 'shared' (num_experts multiplied by pool_size), but experts
      are NOT shared between layers. Each layer independently owns pool_size * num_experts
      experts. This increases total model capacity proportionally.
    - 'global': Every MoE layer can route to all experts contributed by every MoE layer.
    - 'grouped': Every MoE layer can route to experts contributed by its adjacent group.
    - 'sliding': Every MoE layer can route to experts contributed by a centered layer window.
    - 'hyper': Hyper Experts mode. Each layer keeps its own router. When pool_size <= 1
      (default), all MoE layers share one global expert pool; num_moe_experts is the total
      pool size. When pool_size > 1, layers are grouped into groups of pool_size, each
      group sharing its own expert pool; num_moe_experts is the per-pool expert count."""

    moe_layer_pool_size: int = 1
    """Number of adjacent MoE layers visible to each MoE layer in sliding pool mode."""

    moe_pool_aux_loss_coeff: float = 0.0
    """Coefficient for pool-level auxiliary load balancing loss. When > 0 in the hyper
    expert pool mode, balances expert usage across all layers sharing the
    same pool by using a cross-layer averaged token-to-expert distribution. Unlike per-layer
    aux loss which requires each layer to balance independently, this allows experts to
    specialize for specific layers as long as global utilization is balanced."""
    ...
    moe_norm_routing: bool = False
    """Use Norm routing: L2-normalize logits, apply ReLU, then scale by a learnable parameter.
    Selects top-k experts from the resulting non-negative scores."""

    moe_norm_routing_init_method: str = "monte_carlo"
    """Initialization method for the NormRouter scale factor. 'one' sets scale_initial=1;
    'monte_carlo' estimates it via Monte Carlo sampling for the given num_experts and top_k."""

    moe_norm_routing_eps: float = 1e-8
    """Epsilon for numerical stability in L2 normalization of NormRouter."""

    moe_hash_routing: bool = False
    """Enable hash-based routing for a subset of MoE layers. Layers with index >=
    moe_hash_router_start_layer use a parameter-free hash router instead of a learned router."""

    moe_hash_router_start_layer: Optional[int] = None
    """First transformer layer index (0-based) that uses hash routing. All MoE layers at or
    above this index use HashRouter. If None, defaults to ceil(num_layers * 2/3)."""

    moe_hash_router_seed: int = 42
    """Seed for the hash router's multiplicative hash function."""
    ...
    ##################
    # MoD (Mixture of Depths) related
    ##################
    mod_enabled: bool = False
    """Enable Mixture of Depths (MoD) routing for selective token computation."""

    mod_capacity_factor: float = 0.125
    """Fraction of tokens to process through the full computation path.
    Default 0.125 means 12.5% of tokens are selected for processing."""

    mod_aux_loss_coeff: float = 1e-2
    """Scaling coefficient for the MoD router auxiliary loss (BCE)."""

    mod_layer_freq: int = 2
    """Frequency of applying MoD. Default 2 means apply MoD every 2nd layer.
    1 means apply MoD to all layers."""

    mod_stacking_strategy: str = 'frequency'
    """Strategy for stacking MoE and MoDE layers. Options: 'frequency' (use mod_layer_freq),
    'first_half_moe' (first half MoE, second half MoDE), 'first_half_mode' (first half MoDE, second half MoE),
    'last_half_mode' (same as first_half_moe), 'custom' (use mod_layer_pattern)."""

    mod_layer_pattern: Optional[list] = None
    """Custom pattern for MoD layer selection when mod_stacking_strategy='custom'.
    List of 0s and 1s where 1=MoDE layer, 0=standard MoE. Length should match number of MoE layers."""

    staged_mode_enabled: bool = False
    """Enable Staged MoDE (Mixture of Depths + Experts) which combines
    MoD routing with MoE for selective computation."""

    staged_mode_moe_router_topk: Optional[int] = None
    """Override moe_router_topk for Staged MoDE layers only. If None, use
    moe_router_topk. Semantics follow moe_router_topk (and will be scaled by
    moe_granularity if set)."""

    staged_mode_num_experts: Optional[int] = None
    """Override num_experts for Staged MoDE layers only. If None, use
    num_moe_experts from the global MoE configuration."""

    ###############################
    # Integrated MoDE (single router with N+1 outputs: N real experts + 1 Null Expert)
    ###############################
    integrated_mode_enabled: bool = False
    """Enable Integrated MoDE which uses a single router with N+1 outputs where
    index N is the Null Expert (zero MLP output contribution)."""

    integrated_mode_null_expert_aux_loss_coeff: float = 0.0
    """Coefficient for the Null Expert auxiliary loss that encourages the null
    expert selection ratio to match the target ratio."""

    integrated_mode_null_expert_target_ratio: float = 0.5
    """Target ratio of tokens that should be routed to the Null Expert.
    Used by the Null Expert auxiliary loss to control skip ratio."""

    integrated_mode_null_in_load_balance: bool = False
    """Whether to include the Null Expert in the standard load balancing loss.
    If False (default), Null Expert is excluded from aux_loss computation."""
    ...
    def __post_init__(self):
        ...
        if self.moe_norm_routing:
            if self.moe_relu_routing:
                raise ValueError(
                    "moe_norm_routing is mutually exclusive with: moe_relu_routing."
                )
            if self.moe_norm_routing_init_method not in ("one", "monte_carlo"):
                raise ValueError(
                    f"moe_norm_routing_init_method must be 'one' or 'monte_carlo', "
                    f"got '{self.moe_norm_routing_init_method}'"
                )

        if self.moe_hash_routing:
            import math

            if self.moe_hash_router_start_layer is None:
                self.moe_hash_router_start_layer = math.ceil(self.num_layers * 2 / 3)
        ...
        if (
            ...
            # NormRouter does not use softmax, so this check is skipped.
        ...
        # Fine-grained MoE (ReMoE) configuration
