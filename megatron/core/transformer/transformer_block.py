...
class TransformerBlock(GraphableMegatronModule, MegatronModule):
    ...
    def _build_layers(self):
        ...
        # For hyper expert pool mode, share experts progressively during
        # construction to avoid allocating num_layers copies of the full expert
        # pool on GPU simultaneously (which would OOM for large models).
        pool_mode = getattr(self.config, 'moe_expert_pool_mode', 'shared')
        pool_size = getattr(self.config, 'moe_expert_pool_size', 1)
        progressive_hyper = (pool_mode == 'hyper')

        if progressive_hyper:
            layers = []
            leader_experts = {}  # group_key -> experts module
            moe_idx = 0
            for i, layer_spec in enumerate(self.submodules.layer_specs):
                layer = build_layer(layer_spec, i + 1)
                mlp = getattr(layer, 'mlp', None)
                if mlp is not None and hasattr(mlp, 'experts') and mlp.experts is not None:
                    group_key = (moe_idx // pool_size) * pool_size if pool_size > 1 else 0
                    if group_key not in leader_experts:
                        leader_experts[group_key] = mlp.experts
                    else:
                        mlp.experts = leader_experts[group_key]
                    moe_idx += 1
                layers.append(layer)
        ...
        # Expert pool sharing: share experts between groups of adjacent MoE layers.
        # For hyper mode this is mostly a no-op (already shared above) but still
        # runs to set up layer affinity, logging, and other pool-mode features.
        self._setup_expert_pool_sharing()
    ...
    def _setup_expert_pool_sharing(self):
        """Setup expert pool sharing between groups of adjacent MoE layers.

        For 'shared' mode (Approach A): layers in each pool group share the same
        experts module. The first layer's experts are used, and subsequent layers
        point to the same module.
        """
        pool_size = getattr(self.config, 'moe_expert_pool_size', 1)
        pool_mode = getattr(self.config, 'moe_expert_pool_mode', 'shared')
        layer_pool_size = getattr(self.config, 'moe_layer_pool_size', 1)
        if pool_size <= 1 and not is_generalized_expert_pool_mode(pool_mode) and pool_mode != 'hyper':
            return

        # Collect MoE layers (those with mlp.experts attribute)
        moe_layers = []
        for layer in self.layers:
            if getattr(layer, 'is_moe_layer', False) and hasattr(layer, 'mlp'):
                moe_layers.append(layer)

        if len(moe_layers) < 1:
            return

        if is_generalized_expert_pool_mode(pool_mode):
            visible_windows = get_moe_pool_windows(
                len(moe_layers),
                pool_mode,
                pool_size=pool_size,
            ...
            if pool_mode == 'grouped':
                for group_start in range(0, len(moe_layers), pool_size):
                    group = moe_layers[group_start:group_start + pool_size]
                    if not group:
                        continue
                    leader_experts = group[0].mlp.experts
                    total_slots = pool_size * group[0].mlp._base_num_local_experts
                    valid_slots = len(group) * group[0].mlp._base_num_local_experts
                    slot_mask = torch.tensor(
                        [True] * valid_slots + [False] * max(0, total_slots - valid_slots),
                        dtype=torch.bool,
                    )
                    for group_offset, layer in enumerate(group):
                        layer.mlp.experts = leader_experts
                        layer.mlp._pool_visible_moe_indices = visible_windows[
                            group_start + group_offset
                        ]
                        layer.mlp._pool_slot_refs = []
                        layer.mlp._pool_slot_mask = slot_mask
            else:
                leader_experts = moe_layers[0].mlp.experts
                total_slots = len(moe_layers) * moe_layers[0].mlp._base_num_local_experts
                for layer_idx, layer in enumerate(moe_layers):
                    visible_indices = visible_windows[layer_idx]
                    slot_mask = torch.zeros(total_slots, dtype=torch.bool)
                    for visible_idx in visible_indices:
                        slot_start = visible_idx * layer.mlp._base_num_local_experts
                        slot_end = slot_start + layer.mlp._base_num_local_experts
                        slot_mask[slot_start:slot_end] = True
                    layer.mlp.experts = leader_experts
                    layer.mlp._pool_visible_moe_indices = visible_indices
                    layer.mlp._pool_slot_refs = []
                    layer.mlp._pool_slot_mask = slot_mask

            # Pool-level auxiliary load balancing loss for generalized pool modes
            pool_aux_coeff = getattr(self.config, 'moe_pool_aux_loss_coeff', 0.0)
            if pool_aux_coeff > 0:
                from megatron.core.transformer.moe.moe_utils import PoolAuxLossAccumulator

                num_experts_cfg = getattr(self.config, 'num_moe_experts', 0)
                if pool_mode == 'grouped':
                    for group_start in range(0, len(moe_layers), pool_size):
                        group = moe_layers[group_start:group_start + pool_size]
                        acc = PoolAuxLossAccumulator(
                            num_experts=num_experts_cfg,
                            num_moe_layers=len(group),
                            coeff=pool_aux_coeff,
                            topk=self.config.moe_router_topk,
                            device=torch.cuda.current_device(),
                        )
                        for layer in group:
                            layer.mlp.router._pool_aux_loss_accumulator = acc
                else:
                    acc = PoolAuxLossAccumulator(
                        num_experts=num_experts_cfg,
            ...
            if get_pg_rank(pp_group) == 0:
                ...
                pool_aux_str = ""
                if pool_aux_coeff > 0:
                    pool_aux_str = f", pool aux loss coeff={pool_aux_coeff}"
                logger.info(
                    f"Expert pool sharing ({pool_mode}): {len(moe_layers)} MoE layers "
                    f"configured with nominal pool size {nominal_pool_layers}"
                    f"{pool_aux_str}"
                )
            return

        # Hyper Experts mode: MoE layers share expert pools with per-layer routers.
        # Each layer was built with num_experts experts and its own router.
        # When pool_size <= 1 (default): all layers share one global pool.
        # When pool_size > 1: layers are grouped into groups of pool_size,
        # each group sharing its own expert pool.
        if pool_mode == 'hyper':
            if pool_size > 1:
                # Grouped hyper: share experts within groups of pool_size layers
                for group_start in range(0, len(moe_layers), pool_size):
                    group = moe_layers[group_start:group_start + pool_size]
                    if len(group) >= 2:
                        leader_experts = group[0].mlp.experts
                        for follower in group[1:]:
                            follower.mlp.experts = leader_experts
            else:
                # Global hyper: all layers share one pool
                if len(moe_layers) >= 2:
                    leader_experts = moe_layers[0].mlp.experts
                    for follower in moe_layers[1:]:
                        follower.mlp.experts = leader_experts
            # Pool-level auxiliary load balancing loss
            pool_aux_coeff = getattr(self.config, 'moe_pool_aux_loss_coeff', 0.0)
            if pool_aux_coeff > 0:
                from megatron.core.transformer.moe.moe_utils import PoolAuxLossAccumulator

                num_experts = getattr(self.config, 'num_moe_experts', 0)
                if pool_size > 1:
                    # Grouped hyper: one accumulator per group
                    for group_start in range(0, len(moe_layers), pool_size):
                        group = moe_layers[group_start:group_start + pool_size]
                        acc = PoolAuxLossAccumulator(
                    ...
                    # Global hyper: one accumulator for all layers
                    acc = PoolAuxLossAccumulator(
                        num_experts=num_experts,
                        num_moe_layers=len(moe_layers),
                        coeff=pool_aux_coeff,
            ...
            if get_pg_rank(pp_group) == 0:
                ...
                if pool_aux_coeff > 0:
                    pool_aux_str = f", pool aux loss coeff={pool_aux_coeff}"
                if pool_size > 1:
                    num_groups = (len(moe_layers) + pool_size - 1) // pool_size
                    logger.info(
                        f"Hyper Experts (grouped): {len(moe_layers)} MoE layers "
                        f"grouped into {num_groups} pools of {pool_size} layers, "
                        f"each pool with {num_experts} experts"
                        f"{pool_aux_str}"
                    )
                else:
                    logger.info(
                        f"Hyper Experts: {len(moe_layers)} MoE layers sharing "
                        f"{num_experts} global experts"
                        f"{pool_aux_str}"
                    )
            return

        # Group adjacent MoE layers by pool_size
        for group_start in range(0, len(moe_layers), pool_size):
            group = moe_layers[group_start:group_start + pool_size]
            if len(group) < 2:
                continue
            ...
            if pool_mode == 'shared':
                # Approach A: share the first layer's experts module with all others.
                # All layers were built with pool_size * num_experts, so they each
                # have a full set of pooled experts. We share to save memory.
                leader_experts = group[0].mlp.experts
                for follower in group[1:]:
                    follower.mlp.experts = leader_experts
        ...
        if get_pg_rank(pp_group) == 0:
            ...
            logger.info(
                f"Expert pool sharing ({pool_mode}): {len(moe_layers)} MoE layers "
                f"grouped into {num_groups} pools of size {pool_size}"
            )
