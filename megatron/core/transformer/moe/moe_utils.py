...
GENERALIZED_EXPERT_POOL_MODES = {"global", "grouped", "sliding"}


def is_generalized_expert_pool_mode(pool_mode: Optional[str]) -> bool:
    """Whether the configured expert pool mode uses the new generalized sharing logic."""
    return pool_mode in GENERALIZED_EXPERT_POOL_MODES
...
def get_num_moe_layers(
    ...
    """Compute the number of MoE layers implied by a model configuration."""
...
def get_moe_pool_nominal_layer_count(
    num_moe_layers: int,
    pool_mode: Optional[str],
    pool_size: int = 1,
    layer_pool_size: int = 1,
) -> int:
    """Return the nominal number of MoE layers represented in each pooled router slot space."""
    if pool_mode == "global":
        return max(1, num_moe_layers)
    if pool_mode == "grouped":
        return max(1, pool_size)
    if pool_mode == "sliding":
        # Sliding windows overlap, so the clean implementation is a single global bank with
        # per-layer visibility masks rather than a different expert-bank shape per layer.
        return max(1, num_moe_layers)
    return 1


def get_moe_pool_windows(
    num_moe_layers: int,
    pool_mode: Optional[str],
    pool_size: int = 1,
    layer_pool_size: int = 1,
) -> List[List[int]]:
    """Return the visible MoE-layer indices for each MoE layer under the configured pool mode."""
    if num_moe_layers <= 0:
        return []

    if pool_mode == "global":
        visible = list(range(num_moe_layers))
        return [visible[:] for _ in range(num_moe_layers)]

    if pool_mode == "grouped":
        windows = []
        for layer_idx in range(num_moe_layers):
            group_start = (layer_idx // pool_size) * pool_size
            group_end = min(num_moe_layers, group_start + pool_size)
            windows.append(list(range(group_start, group_end)))
        return windows

    if pool_mode == "sliding":
        left_span = max(0, (layer_pool_size - 1) // 2)
        right_span = max(0, layer_pool_size - left_span - 1)
        windows = []
        for layer_idx in range(num_moe_layers):
            start = max(0, layer_idx - left_span)
            end = min(num_moe_layers, layer_idx + right_span + 1)
            windows.append(list(range(start, end)))
        return windows

    return [[layer_idx] for layer_idx in range(num_moe_layers)]
...
def mod_load_balancing_loss_func(
    router_logits: torch.Tensor,
    selected_mask: torch.Tensor,
    mod_aux_loss_coeff: float,
):
    """Calculate the auxiliary loss for MoD (Mixture of Depths) routing using BCE.

    This loss treats top-k selections as binary targets and encourages the router
    to push selected tokens above 0.5 and unselected tokens below 0.5.

    Args:
        router_logits (torch.Tensor): Router logits [seq_len, batch_size, 1] or [seq_len, batch_size]
        selected_mask (torch.Tensor): Boolean mask of selected tokens [seq_len, batch_size]
        mod_aux_loss_coeff (float): Coefficient for the auxiliary loss

    Returns:
        torch.Tensor: The auxiliary loss for routing (scalar)
    """
    if router_logits.dim() == 3:
        router_logits = router_logits.squeeze(-1)

    targets = selected_mask.float()
    aux_loss = torch.nn.functional.binary_cross_entropy_with_logits(router_logits, targets)

    return aux_loss * mod_aux_loss_coeff
...
class PoolAuxLossAccumulator:
    """Accumulates routing stats across MoE layers sharing an expert pool for pool-level
    load balancing loss.

    Uses a one-step-behind scheme: each layer computes its per-layer contribution to the
    pool loss using ``global_tokens_per_expert`` from the previous micro-batch. This avoids
    cross-layer tensor references that would break activation checkpointing.

    Mathematically equivalent to:
        global_f = avg_l(f_l),  global_P = avg_l(P_l)
        pool_loss = coeff * E * dot(global_f, global_P)

    because sum_l(dot(global_f, P_l)) / L = dot(global_f, global_P).
    """
    ...
    def __init__(
        ...
        # Cross-layer averaged tokens_per_expert used as the loss target (detached).
        self.global_tokens_per_expert = torch.zeros(num_experts, device=device)

        # Accumulators for the current micro-batch.
        self._current_tpe_sum = torch.zeros(num_experts, device=device)
    ...
    def accumulate_and_compute_loss(
        ...
        if self._initialized:
            # Per-layer pool loss using previous micro-batch's global_f:
            ...
            # is updated via copy_() later in this call.
            global_tpe = self.global_tokens_per_expert.clone()
            pool_loss = torch.sum(global_tpe * aggregated_probs) * (
                self.num_experts * self.coeff / (self.topk * T * T * L)
            )

        # When all layers have contributed, update global_tokens_per_expert for next cycle.
        if self._current_count == self.num_moe_layers:
            new_tpe = self._current_tpe_sum / self.num_moe_layers
            self.global_tokens_per_expert.copy_(new_tpe)

            self._current_tpe_sum.zero_()
            self._current_count = 0
...
# Global tracker for depth router weight statistics
_DEPTH_ROUTER_WEIGHT_STATS_TRACKER = {}


def get_depth_router_weight_stats_tracker():
    """Return the depth router weight statistics tracker."""
    global _DEPTH_ROUTER_WEIGHT_STATS_TRACKER
    return _DEPTH_ROUTER_WEIGHT_STATS_TRACKER


def save_depth_router_weight_stats(
    weights: torch.Tensor,
    layer_number: int,
    num_layers: int,
    num_histogram_bins: int = 20,
):
    """Save depth router weight statistics for logging.

    Args:
        weights: Router weights [seq_len, batch_size, 1] from sigmoid
        layer_number: Layer index (1-indexed)
        num_layers: Total number of layers
        num_histogram_bins: Number of bins for histogram
    """
    if layer_number is None:
        return

    tracker = get_depth_router_weight_stats_tracker()
    weights_flat = weights.detach().view(-1).float()
    layer_key = f"layer_{layer_number}"

    if layer_key not in tracker:
        device = weights.device
        tracker[layer_key] = {
            "weight_sum": torch.tensor(0.0, device=device),
            "weight_count": torch.tensor(0, dtype=torch.long, device=device),
            "above_half_count": torch.tensor(0, dtype=torch.long, device=device),
            "below_half_count": torch.tensor(0, dtype=torch.long, device=device),
            "histogram_counts": torch.zeros(num_histogram_bins, dtype=torch.long, device=device),
        }

    stats = tracker[layer_key]
    stats["weight_sum"] += weights_flat.sum()
    stats["weight_count"] += weights_flat.numel()
    stats["above_half_count"] += (weights_flat > 0.5).sum()
    stats["below_half_count"] += (weights_flat < 0.5).sum()

    # Update histogram (bins from 0 to 1)
    hist = torch.histc(weights_flat, bins=num_histogram_bins, min=0.0, max=1.0)
    stats["histogram_counts"] += hist.long()


def reduce_depth_router_weight_stats_across_ranks():
    """Reduce depth router weight statistics across all ranks."""
    tracker = get_depth_router_weight_stats_tracker()

    for layer_key, stats in tracker.items():
        for key in ["weight_sum", "weight_count", "above_half_count",
                    "below_half_count", "histogram_counts"]:
            # Reduce across pipeline parallel
            torch.distributed.all_reduce(
                stats[key], group=parallel_state.get_pipeline_model_parallel_group()
            )
            # Reduce across data parallel
            torch.distributed.all_reduce(
                stats[key], group=parallel_state.get_data_parallel_group(with_context_parallel=True)
            )


def clear_depth_router_weight_stats_tracker():
    """Clear the depth router weight statistics tracker."""
    tracker = get_depth_router_weight_stats_tracker()
    for layer_key in tracker:
        for key in tracker[layer_key]:
            tracker[layer_key][key].zero_()


def track_depth_router_weight_metrics(
    iteration: int,
    writer,
    wandb_writer=None,
    ...
    """Track and log depth router weight metrics to tensorboard/wandb."""
    reduce_depth_router_weight_stats_across_ranks()
    tracker = get_depth_router_weight_stats_tracker()
    ...
    for layer_key, stats in sorted(tracker.items()):
        layer_num = int(layer_key.split("_")[1])
        count = stats["weight_count"].item()
        if count == 0:
            continue

        weight_sum = stats["weight_sum"].item()
        above_half = stats["above_half_count"].item()
        below_half = stats["below_half_count"].item()
        hist_counts = stats["histogram_counts"]

        mean_weight = weight_sum / count
        pct_above_half = (above_half / count) * 100
        pct_below_half = (below_half / count) * 100

        total_above_half += above_half
        total_below_half += below_half
        total_count += count
        total_weight_sum += weight_sum

        if aggregated_histogram is None:
            aggregated_histogram = hist_counts.clone().float()
        else:
            aggregated_histogram += hist_counts.float()

        # Per-layer logging
        if per_layer_logging and wandb_writer:
            wandb_writer.log({
                f"depth_router/layer_{layer_num}/mean_weight": mean_weight,
                f"depth_router/layer_{layer_num}/pct_above_0.5": pct_above_half,
                f"depth_router/layer_{layer_num}/pct_below_0.5": pct_below_half,
            }, iteration)

    # Global statistics
    if total_count > 0:
        global_mean_weight = total_weight_sum / total_count
        global_pct_above_half = (total_above_half / total_count) * 100
        global_pct_below_half = (total_below_half / total_count) * 100

        if writer:
            writer.add_scalar("depth_router/mean_weight", global_mean_weight, iteration)
            writer.add_scalar("depth_router/pct_above_0.5", global_pct_above_half, iteration)
            writer.add_scalar("depth_router/pct_below_0.5", global_pct_below_half, iteration)
            # Add histogram to tensorboard
            if aggregated_histogram is not None:
                writer.add_histogram("depth_router/weight_histogram",
                                    aggregated_histogram.cpu(), iteration)

        if wandb_writer:
            wandb_writer.log({
                "depth_router/mean_weight": global_mean_weight,
                "depth_router/pct_above_0.5": global_pct_above_half,
                "depth_router/pct_below_0.5": global_pct_below_half,
            }, iteration)

            # Log histogram to wandb
            if aggregated_histogram is not None:
                bin_edges = [i / num_histogram_bins for i in range(num_histogram_bins + 1)]
                counts = aggregated_histogram.cpu().numpy()
                wandb_writer.log({
                    "depth_router/weight_histogram": wandb_writer.Histogram(
                        np_histogram=(counts, bin_edges)
                    )
                }, iteration)

    clear_depth_router_weight_stats_tracker()


# Global tracker for Integrated MoDE Null Expert statistics
_INTEGRATED_MODE_NULL_EXPERT_STATS_TRACKER = {}


def get_integrated_mode_null_expert_stats_tracker():
    """Return the Integrated MoDE Null Expert statistics tracker."""
    global _INTEGRATED_MODE_NULL_EXPERT_STATS_TRACKER
    return _INTEGRATED_MODE_NULL_EXPERT_STATS_TRACKER


def save_integrated_mode_null_expert_stats(
    null_ratio: torch.Tensor,
    layer_number: int,
    num_layers: int,
):
    """Save Integrated MoDE Null Expert selection statistics for logging.

    Args:
        null_ratio: Ratio of tokens routed to Null Expert (scalar tensor).
        layer_number: Layer index (1-indexed).
        num_layers: Total number of layers.
    """
    if layer_number is None:
    ...
    tracker = get_integrated_mode_null_expert_stats_tracker()
    layer_key = f"layer_{layer_number}"

    if layer_key not in tracker:
        device = null_ratio.device
        tracker[layer_key] = {
    ...
    stats = tracker[layer_key]
    stats["null_ratio_sum"] += null_ratio.detach()
    stats["count"] += 1


def reduce_integrated_mode_null_expert_stats_across_ranks():
    """Reduce Integrated MoDE Null Expert statistics across all ranks."""
    tracker = get_integrated_mode_null_expert_stats_tracker()

    for layer_key, stats in tracker.items():
        for key in ["null_ratio_sum", "count"]:
            # Reduce across pipeline parallel
            torch.distributed.all_reduce(
                stats[key], group=parallel_state.get_pipeline_model_parallel_group()
            )
...
def clear_integrated_mode_null_expert_stats_tracker():
    """Clear the Integrated MoDE Null Expert statistics tracker."""
    tracker = get_integrated_mode_null_expert_stats_tracker()
    for layer_key in tracker:
        for key in tracker[layer_key]:
            tracker[layer_key][key].zero_()
...
def track_integrated_mode_metrics(
    iteration: int,
    writer,
    wandb_writer=None,
    per_layer_logging: bool = False,
):
    """Track and log Integrated MoDE Null Expert metrics to tensorboard/wandb.

    Args:
        iteration: Current training iteration.
        writer: TensorBoard writer.
        wandb_writer: Weights & Biases writer (optional).
        per_layer_logging: Whether to log per-layer metrics.
    """
    reduce_integrated_mode_null_expert_stats_across_ranks()
    tracker = get_integrated_mode_null_expert_stats_tracker()
    ...
    for layer_key, stats in sorted(tracker.items()):
        ...
        null_ratio_sum = stats["null_ratio_sum"].item()
        mean_null_ratio = null_ratio_sum / count

        total_null_ratio_sum += null_ratio_sum
        total_count += count
        layer_ratios[layer_num] = mean_null_ratio

        # Per-layer logging
        if per_layer_logging:
            if writer:
                writer.add_scalar(
                    f"integrated_mode/layer_{layer_num}/null_expert_ratio",
                    mean_null_ratio,
                    iteration,
                )
            if wandb_writer:
                wandb_writer.log({
                    f"integrated_mode/layer_{layer_num}/null_expert_ratio": mean_null_ratio,
                }, iteration)

    # Global statistics
    if total_count > 0:
        global_null_ratio = total_null_ratio_sum / total_count

        if writer:
            writer.add_scalar("integrated_mode/null_expert_ratio", global_null_ratio, iteration)

        if wandb_writer:
            wandb_writer.log({
                "integrated_mode/null_expert_ratio": global_null_ratio,
            }, iteration)

    clear_integrated_mode_null_expert_stats_tracker()
