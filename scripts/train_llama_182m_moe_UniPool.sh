...
set -euo pipefail
####
# UniPool 182m: All MoE layers share one global expert pool with per-layer routers.
# Active/dense-equivalent setting: default uses M = 8 * L = 96 global experts.
# Total expert params match 8E/top-1 vanilla MoE (12 layers * 8 experts/layer).
# Per-token active FFN compute matches dense: top-1 selects one dense-width expert per layer.
####
...
POOL_AUX=${7:-"1e-2"}   # set to coeff to enable pool-level aux loss, empty to disable
POOL_SIZE=${8:-""}       # set to N (e.g. "6") for grouped UniPool: every N layers share one expert pool, empty for global
...
PROJECT_NAME=${9:-"unipool-182m-${NUM_EXPERTS}e${LAYERS_TAG}${TOPK_TAG}${ROUTER_TAG}${LAYER_AUX_TAG}${POOL_AUX_TAG}${POOL_SIZE_TAG}-cc"}
