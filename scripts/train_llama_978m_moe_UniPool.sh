...
set -euo pipefail
####
# UniPool 978m: All MoE layers share one global expert pool with per-layer routers.
# Uses NormRouter (default): L2-normalize + ReLU + learnable scale routing.
...
TRAIN_ITERS=${2:-"60000"}
SAVE_INTERVAL=${10:-"2000"}
SAVE_RETAIN_INTERVAL=${11:-"6000"}
...
PROJECT_NAME=${9:-"unipool-978m-${NUM_EXPERTS}e${LAYERS_TAG}${TOPK_TAG}${ROUTER_TAG}${LAYER_AUX_TAG}${POOL_AUX_TAG}${POOL_SIZE_TAG}-cc"}
