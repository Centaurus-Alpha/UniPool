...
set -euo pipefail
####
# UniPool 830m (48 layers): All MoE layers share one global expert pool with per-layer routers.
...
POOL_SIZE=${8:-""}       # set to N for grouped UniPool: every N layers share one expert pool, empty for global
EP_SIZE=${14:-"1"}       # expert-model-parallel size (must divide NUM_EXPERTS)
EXIT_DURATION_MIN=${15:-""} # wall-clock time budget in minutes; on first iter past the budget, save (if not just saved) and exit
...
PROJECT_NAME=${9:-"unipool-830m-${NUM_EXPERTS}e${LAYERS_TAG}${TOPK_TAG}${EP_TAG}-mb${MICRO_BATCH_SIZE}${ROUTER_TAG}${LAYER_AUX_TAG}${POOL_AUX_TAG}${POOL_SIZE_TAG}-cc"}
